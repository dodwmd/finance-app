...
class BankAccountManagementTest extends TestCase
{
    ...
    {
        $accountData = [
            'name' => 'My Main Bank', // Internal name for the record
    ...
    public function test_bank_account_creation_fails_with_missing_required_fields(): void
    {
        ...
        $response->assertSessionHasErrors(['name', 'type', 'opening_balance', 'account_name']);

        $response = $this->actingAs($this->user)->post(route('bank-accounts.store'), ['name' => 'Test', 'account_name' => 'Test Account']);
        $response->assertSessionHasErrors(['type', 'opening_balance']);

        $response = $this->actingAs($this->user)->post(route('bank-accounts.store'), ['name' => 'Test', 'account_name' => 'Test Account', 'type' => 'bank']);
    ...
    {
        $otherUser = User::factory()->create();
        $otherUserAccount = BankAccount::factory()->for($otherUser)->create(['name' => 'Other User Original Name']);
        $updateData = ['name' => 'Attempted Update Name'];

        $response = $this->actingAs($this->user)->put(route('bank-accounts.update', $otherUserAccount), $updateData);
        $response->assertForbidden();
        $this->assertDatabaseHas('bank_accounts', ['name' => 'Other User Original Name']);
    }

    public function test_bank_account_update_fails_with_missing_required_fields(): void
    {
        $account = BankAccount::factory()->for($this->user)->create();
        $response = $this->actingAs($this->user)->put(route('bank-accounts.update', $account), ['name' => '']);
        $response->assertSessionHasErrors('name');

        $response = $this->actingAs($this->user)->put(route('bank-accounts.update', $account), ['type' => '']);
        $response->assertSessionHasErrors('type');
    }
